            ...
            <div class="row">
                ...
                <div class="col-md-9 sidebar_tile">
                    <ul class="met-column-nav-ul">
                        <li><a href="{{$nav['url_status']==1?url($nav['url']):$nav['link']}}" target="{{$nav['target']==1?'_blank':'_self'}}" class="link"  title="全部">全部</a></li>
                        @foreach($nav['child'] as $v)
                            <li>
                                <a href="{{$v['url_status']==1?url($v['url'].'/'.$v['id']):$v['link']}}" target="{{$v['target']==1?'_blank':'_self'}}" title="{{$v['name']}}" class="link @if($id ==$v['id'] ) active @endif ">{{$v['name']}}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
